...
def mock_pricing_service() -> MagicMock:
    """Mock the PricingService class with async methods."""
    mock = MagicMock(spec=PricingService)
    mock.predict_price = AsyncMock(return_value=MagicMock(id=1, predicted_price=123456.78))
    mock.predict_price_batch = AsyncMock(
        return_value=[
...
@pytest.mark.asyncio
async def test_predict_price_success(mock_pricing_service: MagicMock, payload_single: dict) -> None:
    """Test the /api/v1/price/predict endpoint for a successful prediction."""
    # GIVEN
    app.dependency_overrides[get_pricing_service] = lambda: mock_pricing_service
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
        ...
        mock_pricing_service.predict_price.assert_awaited_once()

    app.dependency_overrides.clear()


@pytest.mark.asyncio
async def test_predict_price_not_found(mock_pricing_service: MagicMock, payload_single: dict) -> None:
    """Test the /api/v1/price/predict endpoint when no result is found."""
    # GIVEN
    mock_pricing_service.predict_price = AsyncMock(side_effect=ValueError('No results found.'))
    app.dependency_overrides[get_pricing_service] = lambda: mock_pricing_service
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
        # WHEN
        response = await client.post('/api/v1/price/predict', json=payload_single)
...
@pytest.mark.asyncio
async def test_batch_predict_success(mock_pricing_service: MagicMock, payload_batch: dict) -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
        ...
        mock_pricing_service.predict_price_batch.assert_awaited_once()

    app.dependency_overrides.clear()


@pytest.mark.asyncio
async def test_batch_predict_not_found(mock_pricing_service: MagicMock, payload_batch: dict) -> None:
    """Test the /api/v1/price/predict/batch endpoint when no result is found."""
    # GIVEN
    mock_pricing_service.predict_price_batch = AsyncMock(side_effect=ValueError('No results found.'))
    app.dependency_overrides[get_pricing_service] = lambda: mock_pricing_service
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
        # WHEN
        response = await client.post('/api/v1/price/predict/batch', json=payload_batch)
